...
private val ScrollButtonSlotSize = 40.dp
 ...
 * more content to scroll to in the first place. Both arrows always reserve their own
 * fixed-size slot so the visible one never shifts into the other's position.
 */
@Composable
fun BoxScope.ScrollToEdgeButtons(listState: LazyListState, scope: CoroutineScope) {
    Column(
        ...
        Box(modifier = Modifier.size(ScrollButtonSlotSize), contentAlignment = Alignment.Center) {
